...
CmdUtils.CreateCommand({
  ...
  preview: function ve_preview(pb, {object: {data}}) {
    ...
    if (data.disabled) div.@class += " disabled";
    var name = <a class="homepage" accesskey="H"/>.appendChild(
      (<img class="icon" src={data.iconURL || this.icon}/>) +
      (<strong class="name">{data.name}</strong>));
    if (data.homepageURL) name.@href = data.homepageURL;
    div.appendChild(name);
    div.appendChild(<span class="version">{data.version}</span>);
    if (data.creator)
      div.appendChild(<div class="creator">{data.creator}</div>);
    if (data.description)
      div.appendChild(<div class="description">{data.description}</div>);
